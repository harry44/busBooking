			...
			<ol class="property-list location">
				...
				<li class="fieldcontain">
					<span id="state-label" class="property-label"><g:message code="location.state.label" default="State" /></span>
						...
						<span class="property-value" aria-labelledby="state-label"><g:link controller="state" action="show" id="${locationInstance?.state?.id}">${locationInstance?.state?.encodeAsHTML()}</g:link></span>
